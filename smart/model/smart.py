...
def joint_scene_from_states(states, object_ids):
    """
    创建Waymo联合场景（仅用于Waymo数据集）
    对于maritime等其他数据集，此函数不会被调用
    """
    if not WAYMO_AVAILABLE:
        raise RuntimeError("Waymo dataset not available. This function is only for Waymo dataset.")
...
class SMART(pl.LightningModule):
    ...
    def __init__(self, model_config) -> None:
        ...
        # 仅在需要地图时初始化地图token；maritime 无地图，跳过以避免无效/缺失文件导致加载失败
        self.map_token = None
        if self.dataset not in ['maritime']:
            self.map_token_traj_path = os.path.join(module_dir, 'tokens/map_traj_token5.pkl')
            try:
                self.init_map_token()
            except Exception:
                # 地图token加载失败时降级为空占位，避免训练中断（无地图场景不会使用）
                self.map_token = {'traj_src': torch.zeros((1, 11, 2), dtype=torch.float)}
        else:
            # 无地图数据集：提供占位结构，后续不会被使用
            self.map_token = {'traj_src': torch.zeros((1, 11, 2), dtype=torch.float)}
        
        # 使用配置文件中的token_path，如果未指定则使用默认值
        if hasattr(model_config, 'token_path') and model_config.token_path:
            self.token_path = model_config.token_path
        elif self.dataset == 'maritime':
            self.token_path = os.path.join(module_dir, 'tokens/maritime_tokens_no_norm.pkl')
        else:
    ...
    def get_trajectory_token(self):
        # 使用 torch.load 并指定 map_location='cpu'（词表通常不需要GPU）
        token_data = torch.load(self.token_path, map_location='cpu')
    ...
    def init_map_token(self):
        ...
        # 使用 torch.load 并指定 map_location='cpu'
        map_token_traj = torch.load(self.map_token_traj_path, map_location='cpu')
    ...
    def training_step(self,
                      ...
                      batch_idx):
        # 只在有地图的数据集中处理地图token
        if self.dataset not in ['maritime'] and 'map_save' in data:
