...
If the script reports that both `metadata` and `scene_info` are missing (only
map structures such as `map_point`/`map_polygon`/`map_save` are present), it
means the preprocessing step never saved the geographic anchors. Regenerate the
dataset with `MaritimeScenePreprocessor` (or re-run your merge step) so that the
per-scene `origin_lat/origin_lon` are persisted alongside the map tokens before
attempting Folium visualization.
